...
use crate::ui_state::{
  ConsoleVisualState, DeckConsoleVisualState, LibraryVisualState, PreferencesState,
};
...
const DSEG7_FONT: &[u8] = include_bytes!("../assets/fonts/DSEG7Classic-Regular.ttf");
const PIXEL_FONT: &[u8] = include_bytes!("../assets/fonts/PixelMplus12-Regular.ttf");
...
pub enum UiAction {
  ...
  RecallCue(u8, f32, Option<f32>),          // deck, position and optional loop end (0..1)
  SetDeckGain(u8, f32),                     // deck, gain 0..1
  LoadFile(u8, String),                     // deck, absolute local file path
  OpenLibraryContextMenu(String, f32, f32), // file path, x, y (logical, top-left origin)
  SetMicEnabled(bool),
  StartRecording,
  StopRecording,
  OpenPreferences,
...
pub(crate) enum MouseEvent {
  Moved(f32, f32),
  PressedPrimary(f32, f32),
  ReleasedPrimary(f32, f32),
  PressedSecondary(f32, f32),
  ReleasedSecondary(f32, f32),
  Wheel(f32),
}

// ── Global shared state ────────────────────────────────────────────────────
static WAVEFORMS: Mutex<[Vec<f32>; 2]> = Mutex::new([Vec::new(), Vec::new()]);
static WAVEFORM_COLORS: Mutex<[Vec<egui::Color32>; 2]> = Mutex::new([Vec::new(), Vec::new()]);
...
fn paint_glow(painter: &egui::Painter, rect: egui::Rect, color: egui::Color32, spread: f32) {
  let steps = 4;
  for i in 1..=steps {
...
fn draw_zoom_waveform(
  ui: &mut egui::Ui,
  samples: &[f32],
  colors: &[egui::Color32],
  visual: &DeckVisualState,
  deck_state: &DeckConsoleVisualState,
  master_tempo: f32,
) {
  ...
  let to_x =
    |pos: f32| -> f32 { rect.min.x + ((pos - view_start) / view_span).clamp(0.0, 1.0) * width };
  ...
  for px in 0..pixel_count {
    ...
    let h = (max_amp * rect.height() * 0.5).max(0.5);
    let color = if !colors.is_empty() && peak_idx < colors.len() {
      colors[peak_idx]
    ...
    painter.rect_filled(
      egui::Rect::from_min_max(
        egui::pos2(x, cy - h),
        egui::pos2((x + bar_w).min(rect.max.x), cy + h),
      ),
      0.0,
      color,
    );
  ...
  for &beat in visual.beats.iter() {
    if beat >= view_start && beat <= view_end {
      painter.vline(
        to_x(beat),
        rect.y_range(),
        egui::Stroke::new(1.0, beat_color),
      );
    }
  }
  ...
  if let Some(intro) = visual.intro {
    painter.vline(
      ...
      egui::Stroke::new(
        2.0,
        egui::Color32::from_rgba_unmultiplied(100, 255, 100, 204),
      ),
    );
  }
  ...
  if let Some(outro) = visual.outro {
    painter.vline(
      ...
      egui::Stroke::new(
        2.0,
        egui::Color32::from_rgba_unmultiplied(255, 255, 100, 204),
      ),
    );
  }
  ...
  if deck_state.loop_enabled && deck_state.loop_start < deck_state.loop_end {
    ...
    if lx2 > lx1 {
      let loop_rect =
        egui::Rect::from_min_max(egui::pos2(lx1, rect.min.y), egui::pos2(lx2, rect.max.y));
      painter.rect_filled(
        loop_rect,
        0.0,
        egui::Color32::from_rgba_unmultiplied(0, 204, 102, 40),
      );
      painter.vline(
        lx1,
        rect.y_range(),
        egui::Stroke::new(1.0, egui::Color32::from_rgba_unmultiplied(0, 204, 102, 180)),
      );
      painter.vline(
        lx2,
        rect.y_range(),
...
fn draw_full_waveform(
  ...
  let to_x = |pos: f32| -> f32 { rect.min.x + (pos / total_frames).clamp(0.0, 1.0) * width };
  ...
  for i in 0..bar_count {
    ...
    let mut max_amp: f32 = 0.0;
    let mut peak_idx = idx_start.min(samples.len().saturating_sub(1));
...
fn draw_level_meter(ui: &mut egui::Ui, peak: f32, peak_hold: f32, width: f32, height: f32) {
  ...
  for i in 0..num_segments {
    ...
    let seg_rect =
      egui::Rect::from_min_size(egui::pos2(rect.min.x, y), egui::vec2(width, seg_h - 1.0));
...
fn draw_volume_slider(ui: &mut egui::Ui, deck: u8, gain: f32, width: f32, height: f32) {
  ...
  painter.rect_stroke(
    handle_rect,
    1.0,
    egui::Stroke::new(1.0, egui::Color32::from_rgb(102, 102, 102)),
    egui::StrokeKind::Outside,
  );
}
...
fn draw_eq_kill_button(ui: &mut egui::Ui, deck: u8, label: &str, band: &'static str, active: bool) {
  ...
  if active {
    paint_gradient_135(
      ...
      draw_rect,
      ORANGE,
      egui::Color32::from_rgb(204, 55, 0),
      3.0,
    );
    painter.rect_stroke(
      draw_rect,
    ...
    painter.rect_stroke(
      draw_rect,
      3.0,
      egui::Stroke::new(1.0, BORDER_MED),
      egui::StrokeKind::Outside,
    );
  }
...
fn draw_cue_button(ui: &mut egui::Ui, deck: u8, enabled: bool) {
  ...
  if enabled {
    paint_gradient_135(
      ...
      egui::Color32::from_rgb(15, 22, 32),
      4.0,
    );
    painter.rect_stroke(
      draw_rect,
    ...
    for row in [row1, row2] {
      ui.horizontal(|ui| {
        ...
        for &beats in row.iter() {
          ...
          draw_loop_button(ui, deck, &label, beats, active, enabled, time);
        }
      });
    }
  });
...
fn draw_loop_button(
  ...
  if active {
    ...
    paint_gradient_135(
      ...
      draw_rect,
      GREEN,
      egui::Color32::from_rgb(0, 153, 68),
      3.0,
    );
    painter.rect_stroke(
      draw_rect,
...
fn draw_deck(
  ...
  waveform: &[f32],
  waveform_colors: &[egui::Color32],
  visual: &DeckVisualState,
) -> egui::Response {
  let has_track = !deck.title.is_empty();
  let border_color = if has_track {
    CYAN
  } else {
    egui::Color32::from_rgb(68, 68, 68)
  };
...
fn draw_rekordbox_cue_buttons(
  ui: &mut egui::Ui,
  deck: u8,
  cues: &[crate::ui_state::DeckCueVisualState],
  enabled: bool,
  align_right: bool,
) {
  let rail_width = ui.available_width();
  ui.allocate_ui_with_layout(
    egui::vec2(rail_width, 20.0),
    if align_right {
    ...
    |ui| {
      ...
      let max_buttons = ((rail_width - 30.0) / 33.0).floor().max(1.0) as usize;
      for cue in cues.iter().take(max_buttons.min(8)) {
        let label = if cue.loop_end.is_some() {
          format!("L{}", cue.label)
        } else {
          cue.label.clone()
        };
        let color = cue
          ...
          .unwrap_or(ORANGE_SEC);
        let button = egui::Button::new(egui::RichText::new(label).size(9.0).color(color))
          .min_size(egui::vec2(30.0, 20.0));
        if ui.add_enabled(enabled, button).clicked() {
          push_action(UiAction::RecallCue(deck, cue.position, cue.loop_end));
        }
      }
      ui.label(egui::RichText::new("CUE").size(9.0).color(TEXT_DIM));
    },
  );
}
...
fn draw_tempo_section(
  ui: &mut egui::Ui,
  state: &ConsoleVisualState,
  visuals: &[DeckVisualState; 2],
) {
...
fn draw_meters_section(
  ...
  let content_w =
    col_eq + sp + col_vol + sp + col_meter + sp + col_meter + sp + col_vol + sp + col_eq;
  ui.horizontal(|ui| {
    ...
    // EQ kills A
    draw_eq_kills_column(
      ui,
      1,
      state.deck_a.eq_high,
      state.deck_a.eq_mid,
      state.deck_a.eq_low,
    );
    ...
    ui.allocate_ui_with_layout(
      ...
      |ui| {
        ...
        ui.add(egui::Label::new(
          egui::RichText::new(pct).size(8.0).color(TEXT_DIM),
        ));
      },
    );
...
fn draw_crossfader(ui: &mut egui::Ui, position: f32) {
  ...
  paint_gradient_h(
    ...
    egui::Rect::from_min_max(
      track_rect.left_top(),
      egui::pos2(mid_x, track_rect.bottom()),
    ),
    ORANGE_SEC,
  ...
  let handle =
    egui::Rect::from_center_size(egui::pos2(sx, rect.center().y), egui::vec2(10.0, 24.0));
  paint_gradient_135(
    painter,
    handle,
    egui::Color32::from_rgb(85, 85, 85),
    ...
    .show(ctx, |ui| {
      ...
      for i in 1u8..=3 {
        ...
        painter.hline(
          ...
          egui::Stroke::new(
            1.0,
            egui::Color32::from_rgba_unmultiplied(0, 212, 255, alpha),
          ),
        );
      }
      ...
      ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
        ...
        {
          let (bar_r, _) = ui.allocate_exact_size(egui::vec2(50.0, 8.0), egui::Sense::hover());
          let p = ui.painter();
          p.rect_filled(
            bar_r,
            4.0,
          ...
          if fill_w > 1.0 {
            ...
            let lh = egui::Rect::from_x_y_ranges(bar_r.min.x..=mid, bar_r.y_range());
            let rh = egui::Rect::from_x_y_ranges(mid..=(bar_r.min.x + fill_w), bar_r.y_range());
        ...
        {
          let (bar_r, _) = ui.allocate_exact_size(egui::vec2(56.0, 6.0), egui::Sense::hover());
          let p = ui.painter();
          p.rect_filled(
            bar_r,
            3.0,
        ...
        let rec_resp = tb_pill_button(
          ui, &rec_label, rec_active, rec_border, rec_dot, rec_text, rec_bg_top, rec_bg_bot, true,
        );
        if rec_resp.clicked() {
          if rec_active {
        ...
        egui::ComboBox::from_id_salt((title, side))
          ...
          .show_ui(ui, |ui| {
            ...
            for channel in 0..max_channels {
              ...
              if ui
                .selectable_label(selected, (channel + 1).to_string())
                .clicked()
              {
      ...
      match PREFS_TAB.load(Ordering::Relaxed) {
        0 => {
          ...
          egui::ComboBox::from_id_salt("pref_device")
            ...
            .show_ui(ui, |ui| {
              if ui
                .selectable_label(draft.audio_device_id.is_none(), "System Default")
                .clicked()
              {
        ...
        1 => {
          ...
          egui::ComboBox::from_id_salt("pref_rec_naming")
            ...
            .show_ui(ui, |ui| {
              if ui
                .selectable_label(draft.recording_naming_strategy == "timestamp", "timestamp")
                .clicked()
              {
          ...
          egui::ComboBox::from_id_salt("pref_rec_format")
            ...
            .show_ui(ui, |ui| {
              if ui
                .selectable_label(draft.recording_format == "wav", "wav")
                .clicked()
              {
                draft.recording_format = "wav".to_owned();
              }
              if ui
                .selectable_label(draft.recording_format == "ogg", "ogg")
                .clicked()
              {
...
fn build_console_ui(ctx: &egui::Context) {
  ...
  let waveform_colors = WAVEFORM_COLORS.lock().unwrap().clone();
  ...
  egui::CentralPanel::default()
    ...
    .show(ctx, |ui| {
      ...
      ui.horizontal_top(|ui| {
        ...
        let left_deck = ui.allocate_ui_with_layout(
        ...
        ui.add_space(gap);
        let right_deck = ui.allocate_ui_with_layout(
          egui::vec2(deck_w, deck_area_h),
          egui::Layout::top_down(egui::Align::Min),
          |ui| {
            draw_deck(
        ...
        let active_drag = { LIBRARY_DRAG_SOURCE.lock().unwrap().clone() };
        if let Some(drag) = active_drag {
          ctx.output_mut(|o| o.cursor_icon = egui::CursorIcon::Grabbing);

          let (released, pointer_pos) =
            ctx.input(|i| (i.pointer.any_released(), i.pointer.interact_pos()));
          if released {
            if let Some(pos) = pointer_pos {
              if left_deck.response.rect.contains(pos) {
                push_action(UiAction::LoadFile(1, drag.file_path.clone()));
              } else if right_deck.response.rect.contains(pos) {
                push_action(UiAction::LoadFile(2, drag.file_path.clone()));
              }
            }
            *LIBRARY_DRAG_SOURCE.lock().unwrap() = None;
          } else if let Some(pos) = pointer_pos {
            let preview = egui::Area::new("library_drag_preview".into())
              .order(egui::Order::Foreground)
              .fixed_pos(pos + egui::vec2(12.0, 12.0))
              .show(ctx, |ui| {
                egui::Frame::popup(ui.style()).show(ui, |ui| {
                  ui.label(
                    egui::RichText::new(drag.title)
                      .size(10.0)
                      .color(TEXT_PRIMARY),
                  );
                  ui.label(egui::RichText::new(drag.track_id).size(9.0).color(TEXT_DIM));
                });
              });
            let _ = preview;
          }
        }
        if LIBRARY_DRAG_SOURCE.lock().unwrap().is_some()
...
///
/// `kind`: 0 = cursor moved, 1/2 = left press/release, 3/4 = right press/release, 5 = wheel.
/// `x`, `y`: logical points, top-left origin, relative to the view's top-left corner.
pub fn push_mouse_event(kind: u8, x: f32, y: f32) {
  let event = match kind {
    1 => MouseEvent::PressedPrimary(x, y),
    2 => MouseEvent::ReleasedPrimary(x, y),
    3 => MouseEvent::PressedSecondary(x, y),
    4 => MouseEvent::ReleasedSecondary(x, y),
    5 => MouseEvent::Wheel(y),
...
pub fn set_waveform_colors(deck_index: usize, colors: Vec<[u8; 3]>) {
  if deck_index > 1 {
    return;
  }
  let mut guard = WAVEFORM_COLORS.lock().unwrap();
  guard[deck_index] = colors
    .into_iter()
...
pub fn set_deck_progress(
  ...
  let current_peak = if deck_index == 0 {
    console.deck_a.peak
  } else {
    console.deck_b.peak
  };
...
pub fn set_deck_markers(
  ...
  guard[deck_index].beats = beats.into_iter().filter(|v| v.is_finite()).collect();
...
fn draw_library_panel(ui: &mut egui::Ui, library: &LibraryVisualState) {
  ...
  egui::Frame::NONE
    ...
    .show(ui, |ui| {
      ...
      {
        let mut selected = LIBRARY_SELECTED_TRACK_ID.lock().unwrap();
        if let Some(id) = selected.as_ref() {
          if !library.tracks.iter().any(|track| track.id == *id) {
            *selected = None;
          }
        }
      }

      let table_w = ui.available_width().max(760.0);
      let bpm_w = 64.0;
      let duration_w = 76.0;
      let rating_w = 82.0;
      let release_w = 100.0;
      let fixed_w = bpm_w + duration_w + rating_w + release_w;
      let free_w = (table_w - fixed_w).max(420.0);
      let title_w = (free_w * 0.32).clamp(160.0, 260.0);
      let artist_w = (free_w * 0.22).clamp(110.0, 190.0);
      let album_w = (free_w * 0.22).clamp(110.0, 190.0);
      ...
      let table = TableBuilder::new(ui)
        .id_salt("rekordbox_library_table")
        .striped(false)
        .resizable(false)
        .sense(egui::Sense::hover())
        .drag_to_scroll(false)
        .cell_layout(egui::Layout::left_to_right(egui::Align::Center))
        .column(Column::exact(title_w))
        .column(Column::exact(artist_w))
        .column(Column::exact(album_w))
        .column(Column::exact(bpm_w))
        .column(Column::exact(duration_w))
        .column(Column::exact(rating_w))
        .column(Column::exact(tags_w))
        .column(Column::exact(release_w))
        .header(20.0, |mut header| {
          header.col(|ui| {
            draw_sort_header(ui, title_w, "TITLE", LibrarySortKey::Title, &mut sort_state)
          });
          header.col(|ui| {
            draw_sort_header(
              ui,
          ...
          header.col(|ui| {
            draw_sort_header(ui, album_w, "ALBUM", LibrarySortKey::Album, &mut sort_state)
          });
          header.col(|ui| draw_sort_header(ui, bpm_w, "BPM", LibrarySortKey::Bpm, &mut sort_state));
          header.col(|ui| {
            draw_sort_header(
              ui,
              duration_w,
          ...
          header
            .col(|ui| draw_sort_header(ui, tags_w, "TAGS", LibrarySortKey::Tags, &mut sort_state));
          header.col(|ui| {
            draw_sort_header(
              ui,
              release_w,
              "RELEASE",
              LibrarySortKey::Release,
              &mut sort_state,
            )
          });
        });
      ...
      let mut selected_track_id = LIBRARY_SELECTED_TRACK_ID.lock().unwrap();
      ...
      table.body(|body| {
        body.rows(22.0, tracks.len(), |mut row| {
          ...
          let title_text = truncate_for_column(&track.title, title_w);
          let mut title_cell = None;
          row.col(|ui| {
            let resp = ui.add_sized(
              [title_w, 18.0],
              egui::Label::new(
                egui::RichText::new(title_text.clone())
                  .size(11.0)
                  .color(TEXT_PRIMARY),
              )
              .sense(egui::Sense::click_and_drag()),
            );
            if title_text != track.title {
              resp.clone().on_hover_text(track.title.clone());
            }
            if resp.drag_started_by(egui::PointerButton::Primary) {
              *LIBRARY_DRAG_SOURCE.lock().unwrap() = Some(LibraryDragSource {
                track_id: track.id.clone(),
                file_path: track.file_path.clone(),
                title: track.title.clone(),
              });
            }
            title_cell = Some(resp);
          });
          let title_cell = title_cell.expect("title cell response");

          let artist_value = if track.artist.is_empty() {
            "-"
          } else {
            &track.artist
          };
          let artist_text = truncate_for_column(artist_value, artist_w);
          let mut artist_cell = None;
          row.col(|ui| {
            let resp = ui.add_sized(
              [artist_w, 18.0],
              egui::Label::new(
                egui::RichText::new(artist_text.clone())
                  .size(10.0)
                  .color(TEXT_DIM),
              )
              .sense(egui::Sense::click_and_drag()),
            );
            if artist_text != artist_value {
              resp.clone().on_hover_text(artist_value.to_owned());
            }
            artist_cell = Some(resp);
          });
          let artist_cell = artist_cell.expect("artist cell response");

          let album_value = if track.album.is_empty() {
            "-"
          } else {
            &track.album
          };
          let album_text = truncate_for_column(album_value, album_w);
          let mut album_cell = None;
          row.col(|ui| {
            let resp = ui.add_sized(
              [album_w, 18.0],
              egui::Label::new(
                egui::RichText::new(album_text.clone())
                  .size(10.0)
                  .color(TEXT_DIM),
              )
              .sense(egui::Sense::click_and_drag()),
            );
            if album_text != album_value {
              resp.clone().on_hover_text(album_value.to_owned());
            }
            album_cell = Some(resp);
          });
          let album_cell = album_cell.expect("album cell response");

          let mut bpm_cell = None;
          row.col(|ui| {
            let bpm = track
              .bpm
              .map(|v| format!("{:.1}", v))
            ...
            bpm_cell = Some(
              ui.add_sized(
                [bpm_w, 18.0],
                egui::Label::new(egui::RichText::new(bpm).size(10.0).color(TEXT_DIM))
                  .sense(egui::Sense::click_and_drag()),
              ),
            );
          });
          let bpm_cell = bpm_cell.expect("bpm cell response");

          let mut duration_cell = None;
          row.col(|ui| {
            let duration = track
              .duration_seconds
            ...
            duration_cell = Some(
              ui.add_sized(
                [duration_w, 18.0],
                egui::Label::new(egui::RichText::new(duration).size(10.0).color(TEXT_DIM))
                  .sense(egui::Sense::click_and_drag()),
              ),
            );
          });
          let duration_cell = duration_cell.expect("duration cell response");

          let mut rating_cell = None;
          row.col(|ui| {
            let rating = track
              .rating
              .map(format_rating_stars)
              .unwrap_or_else(|| "☆☆☆☆☆".to_owned());
            rating_cell = Some(
              ui.add_sized(
                [rating_w, 18.0],
                egui::Label::new(egui::RichText::new(rating).size(10.0).color(TEXT_DIM))
                  .sense(egui::Sense::click_and_drag()),
              ),
            );
          });
          let rating_cell = rating_cell.expect("rating cell response");

          let mut tags_cell = None;
          row.col(|ui| {
            let tags = track
              .tags
              .as_ref()
              ...
              .map(|v| format_tags(v))
              .unwrap_or_else(|| "-".to_owned());
            let tags_text = truncate_for_column(&tags, tags_w);
            tags_cell = Some(
              ui.add_sized(
                [tags_w, 18.0],
                egui::Label::new(egui::RichText::new(tags_text).size(10.0).color(TEXT_DIM))
                  .sense(egui::Sense::click_and_drag()),
              ),
            );
          });
          let tags_cell = tags_cell.expect("tags cell response");
          ...
          row.col(|ui| {
            let release = track
              ...
              .as_ref()
              .filter(|v| !v.is_empty())
              .map(|v| format_release_date(v))
              .unwrap_or_else(|| "-".to_owned());
            release_cell = Some(
              ui.add_sized(
                [release_w, 18.0],
                egui::Label::new(egui::RichText::new(release).size(10.0).color(TEXT_DIM))
                  .sense(egui::Sense::click_and_drag()),
              ),
            );
          });
          let release_cell = release_cell.expect("release cell response");

          let row_response = title_cell
            .clone()
            .union(artist_cell.clone())
            .union(album_cell.clone())
            .union(bpm_cell.clone())
            .union(duration_cell.clone())
            .union(rating_cell.clone())
            .union(tags_cell.clone())
            .union(release_cell.clone());
          if selected {
            ctx.layer_painter(row_response.layer_id).rect_stroke(
              row_response.rect,
              0.0,
              egui::Stroke::new(1.0, CYAN),
          ...
          if cell_responses.iter().any(|resp| resp.clicked()) {
          ...
          if secondary_pressed && cell_responses.iter().any(|resp| resp.hovered()) {
            *selected_track_id = Some(track.id.clone());
            if let Some(position) = ctx.input(|input| input.pointer.interact_pos()) {
              push_action(UiAction::OpenLibraryContextMenu(
          ...
          if cell_responses.iter().any(|resp| resp.double_clicked()) {
            push_action(UiAction::LoadFile(1, track.file_path.clone()));
          }
          if row_response.drag_started_by(egui::PointerButton::Primary) {
            *LIBRARY_DRAG_SOURCE.lock().unwrap() = Some(LibraryDragSource {
              track_id: track.id.clone(),
              file_path: track.file_path.clone(),
              title: track.title.clone(),
...
fn format_rating_stars(rating: i32) -> String {
  let clamped = rating.clamp(0, 5) as usize;
  let mut stars = String::with_capacity(8);
  for _ in 0..clamped {
    stars.push('★');
  }
  for _ in clamped..5 {
...
fn format_duration_mmss(duration_seconds: f32) -> String {
  let total = duration_seconds.max(0.0) as u32;
...
fn draw_sort_header(
  ...
  label: &str,
  key: LibrarySortKey,
  state: &mut LibrarySortState,
) {
  let mut text = label.to_owned();
  if state.key == key {
    text.push_str(if state.ascending { " ^" } else { " v" });
  }
...
fn compare_library_tracks(
  left: &crate::ui_state::LibraryTrackItem,
  right: &crate::ui_state::LibraryTrackItem,
  sort: LibrarySortState,
) -> CmpOrdering {
  match sort.key {
    LibrarySortKey::Title => left
      .title
      .to_ascii_lowercase()
      .cmp(&right.title.to_ascii_lowercase()),
    LibrarySortKey::Artist => left
      .artist
      .to_ascii_lowercase()
      .cmp(&right.artist.to_ascii_lowercase()),
    LibrarySortKey::Album => left
      .album
      .to_ascii_lowercase()
      .cmp(&right.album.to_ascii_lowercase()),
    LibrarySortKey::Bpm => cmp_option_f32(left.bpm, right.bpm),
    LibrarySortKey::Duration => cmp_option_f32(left.duration_seconds, right.duration_seconds),
    LibrarySortKey::Rating => left.rating.unwrap_or(-1).cmp(&right.rating.unwrap_or(-1)),
    LibrarySortKey::Tags => left
      .tags
      .as_deref()
      .unwrap_or("")
      .to_ascii_lowercase()
      .cmp(&right.tags.as_deref().unwrap_or("").to_ascii_lowercase()),
    LibrarySortKey::Release => format_release_date(left.release_date.as_deref().unwrap_or("")).cmp(
      &format_release_date(right.release_date.as_deref().unwrap_or("")),
    ),
  }
}

fn cmp_option_f32(left: Option<f32>, right: Option<f32>) -> CmpOrdering {
  match (left, right) {
    (Some(l), Some(r)) => l.partial_cmp(&r).unwrap_or(CmpOrdering::Equal),
...
fn truncate_for_column(value: &str, width_px: f32) -> String {
  let max_chars = ((width_px - 8.0) / 6.5).floor().max(4.0) as usize;
...
pub(crate) fn run_egui_render_loop(
  ...
  while running.load(Ordering::Relaxed) {
    ...
    raw_input
      .viewports
      .insert(raw_input.viewport_id, viewport_info);
    ...
    // Inject mouse events from the host window
    {
      ...
      for me in events.drain(..) {
        match me {
          ...
          MouseEvent::ReleasedPrimary(x, y) => {
            raw_input.events.push(egui::Event::PointerButton {
              pos: egui::pos2(x, y),
              button: egui::PointerButton::Primary,
              pressed: false,
          ...
          MouseEvent::PressedSecondary(x, y) => {
            raw_input
              .events
              .push(egui::Event::PointerMoved(egui::pos2(x, y)));
            raw_input.events.push(egui::Event::PointerButton {
          ...
          MouseEvent::ReleasedSecondary(x, y) => {
            raw_input.events.push(egui::Event::PointerButton {
              pos: egui::pos2(x, y),
              button: egui::PointerButton::Secondary,
              pressed: false,
