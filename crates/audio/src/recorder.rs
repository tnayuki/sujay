...
impl OggWriter {
  fn new(path: &str, sample_rate: u32) -> Result<Self, String> {
    let f = File::create(path).map_err(|e| format!("Failed to create OGG file: {}", e))?;
    let writer = BufWriter::new(f);
    let sampling_frequency =
      NonZeroU32::new(sample_rate).ok_or_else(|| "Invalid sample rate".to_string())?;
    let channels = NonZeroU8::new(2).ok_or_else(|| "Invalid channel count".to_string())?;
    let mut builder =
      VorbisEncoderBuilder::new_with_serial(sampling_frequency, channels, writer, 0);
    let encoder = builder
      .build()
...
impl RecordingThread {
  ...
  pub fn start_recording(&mut self, path: String, format: RecordingFormat) -> Result<(), String> {
    ...
    if let Some(ref sender) = self.sender {
      sender
        .send(RecordingMessage::Start { path, format })
    ...
    while let Ok(message) = receiver.recv() {
      match message {
        RecordingMessage::Start { path, format } => {
          writer = match format {
            RecordingFormat::Wav => WavWriter::new(&path, sample_rate)
              .ok()
              .map(|w| Box::new(w) as Box<dyn AudioWriter>),
            RecordingFormat::Ogg => OggWriter::new(&path, sample_rate)
              .ok()
              .map(|w| Box::new(w) as Box<dyn AudioWriter>),
          };
        }
